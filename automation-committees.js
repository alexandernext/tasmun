...
const getShortenedTopic = topic => {
  return topic.indexOf("(") !== -1
    ? topic.substring(0, topic.indexOf("(") - 1)
    : topic;
};
...
for (let i = 0; i < arr.length; i++) {
  ...
  const title = getShotenedTitle(lines[0]);
  const topicsIdx = lines.indexOf("Topics");
  const topics = [];
  let j = 1;
  while (topicsIdx + j < lines.length) {
    topics.push(getShortenedTopic(lines[topicsIdx + j]));
    j += 1;
  }
  const topicsString = getListString(topics);
